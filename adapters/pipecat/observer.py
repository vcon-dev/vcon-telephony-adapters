...
LLMTextFrame / LLMFullResponseEndFrame, and EndFrame; builds a
`PipecatConversationState` incrementally; emits a vCon on EndFrame.
...
This module deliberately imports `pipecat` lazily so the rest of the
monorepo doesn't need it. Install Pipecat separately to use this adapter:
...
class VconConversationObserver:
    ...
    NOTE: this class is intentionally NOT a subclass of Pipecat's
    `FrameProcessor` at import time — that would force every user of this
    monorepo to install pipecat-ai. The real subclass is created lazily via
    `as_frame_processor()`.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.builder = builder or PipecatVconBuilder(publisher=publisher, lawful_basis=lawful_basis)
    ...
    def record_user_transcript(self, text: str, *, when: datetime | None = None) -> None:
        self.state.turns.append(
            PipecatTurn(role="user", text=text, start_time=when or datetime.now(UTC))
        )
